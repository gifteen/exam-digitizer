...
9. If text is crossed out / struck through, skip it — it was intentionally deleted.
10. If the page is tilted or rotated, read it correctly regardless of orientation.
11. Do not repeat content. Each sentence appears once. If you find yourself writing the same thing twice, stop.
...
function textToHtml(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const withHighlights = escHtml(trimmed).replace(
      /\[\?\]/g,
      '<mark class="uncertain" title="Unclear in original — click to edit">[?]</mark>'
    );
